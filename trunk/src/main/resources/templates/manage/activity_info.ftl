...
<script type="text/javascript">
    $(document).ready(function() {
        initTable("table", "/manage/webSetting/activityList");
    } );
    ...
    function operateFormatter(value, row, index) {
        if(value == '1') {
            return "<button type='button' class='btn btn-primary' onclick='showEdit(\""+'/manage/webSetting/queryById2?id='+row.id+"\")'>编辑</button>&nbsp;&nbsp;<button type='button' class='btn btn-danger' onclick='showCanleRelease(\""+'/manage/webSetting/release?id='+ row.id+'&status=Y'+ "\")'>取消发布</button>";
        } else {
            return "<button type='button' class='btn btn-primary' onclick='showEdit(\""+'/manage/webSetting/queryById2?id='+row.id+"\")'>编辑</button>&nbsp;&nbsp;<button type='button' class='btn btn-success' onclick='showRelease(\""+'/manage/webSetting/release?id='+ row.id+'&status=N'+ "\")'>发布</button>";
        }
    }
    ...
    function showAdd() {
        layer.open({
            ...
            shade: 0.8,
            area: ['1000px', '80%'],
            content: "/manage/webSetting/queryById2", //iframe的url
            end: function () {
                location.reload();
            }
        });
