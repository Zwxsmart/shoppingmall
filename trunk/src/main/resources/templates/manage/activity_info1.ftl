...
<div class="form-horizontal" role="form">
    <div class="panel-body" style="padding-bottom:0px;">
        <table id="table" style="table-layout: fixed">
            <thead>
            <tr>
                <th data-width="150" data-field="showPicTitle" data-formatter="formatterImg">热门活动图片</th>
                <th data-width="100" data-field="title">热门活动标题</th>
                <#--<th data-width="600" data-field="content">热门活动内容</th>-->
                <th data-width="130" data-field="createDate" data-formatter="formatterDate">热门活动创建时间</th>
        ...
        <div id="toolbar" class="btn-group">
            <button id="btn_add" type="button" class="btn btn-primary" onclick="showAdd();">
                <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>新增热门活动
            </button>
        </div>
    </div>
</body>
...
<script type="text/javascript">
    $(document).ready(function() {
        initTable("table", "/manage/webSetting/activityList1");
    } );
    ...
    function operateFormatter(value, row, index) {
        if(value == '1') {
            return "<button type='button' class='btn btn-primary' onclick='showEdit(\""+'/manage/webSetting/queryById3?id='+row.id+"\")'>编辑</button>&nbsp;&nbsp;<button type='button' class='btn btn-danger' onclick='showCanleRelease(\""+'/manage/webSetting/release?id='+ row.id+'&status=Y'+ "\")'>取消发布</button>";
        } else {
            return "<button type='button' class='btn btn-primary' onclick='showEdit(\""+'/manage/webSetting/queryById3?id='+row.id+"\")'>编辑</button>&nbsp;&nbsp;<button type='button' class='btn btn-success' onclick='showRelease(\""+'/manage/webSetting/release?id='+ row.id+'&status=N'+ "\")'>发布</button>";
        }
    }
    ...
    function showAdd() {
        layer.open({
            ...
            shade: 0.8,
            area: ['1000px', '80%'],
            content: "/manage/webSetting/queryById3", //iframe的url
            end: function () {
                location.reload();
            }
        });
