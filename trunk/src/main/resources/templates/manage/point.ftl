    ...
    <div class="panel-body" style="padding-bottom:0px;">
        <table id="table" style="table-layout: fixed">
            ...
            <tr>
                ...
                <th data-width="130" data-field="createDate" data-formatter="formatterDate">投放点创建时间</th>
        ...
        <div id="toolbar" class="btn-group">
            <button id="btn_add" type="button" class="btn btn-primary" onclick="showAdd();">
                <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>新增投放点
            </button>
        </div>
    </div>
</body>
...
<script type="text/javascript">
    $(document).ready(function() {
        initTable("table", "/manage/webSetting/queryPointList");
    } );
    ...
    function operateFormatter(value, row, index) {
        if(value == '1') {
            return "<button type='button' class='btn btn-primary' onclick='showEdit(\""+'/manage/webSetting/queryById5?id='+row.id+"\")'>编辑</button>&nbsp;&nbsp;" +
                    "<button type='button' class='btn btn-danger' onclick='showCanleRelease(\""+'/manage/webSetting/release2?id='+ row.id+'&status=Y'+ "\")'>冻结</button>";
        } else {
            return "<button type='button' class='btn btn-primary' onclick='showEdit(\""+'/manage/webSetting/queryById5?id='+row.id+"\")'>编辑</button>&nbsp;&nbsp;" +
                    "<button type='button' class='btn btn-success' onclick='showRelease(\""+'/manage/webSetting/release2?id='+ row.id+'&status=N'+ "\")'>激活</button>";
        }
    }
    ...
    function showAdd() {
        layer.open({
            ...
            shade: 0.8,
            area: ['1000px', '80%'],
            content: "/manage/webSetting/queryById5", //iframe的url
            end: function () {
                location.reload();
            }
        });
