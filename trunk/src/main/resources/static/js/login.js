...
$(function(){
    ...
    // 获取登录验证码1
    $(".login_get_code1").click(function() {
        var iphone = $("#phone1").val(); //获取输入框里的手机号码
        var reg = /^1[3578]\d{9}$/;
        if(reg.test(iphone)) {
            var btn = $(this);
            var count = 60;
    ...
    // 获取登录验证码2
    $(".login_get_code2").click(function() {
        var iphone = $("#phone2").val(); //获取输入框里的手机号码
        var reg = /^1[3578]\d{9}$/;
        if(reg.test(iphone)) {
            var btn = $(this);
            var count = 60;
    ...
    // 绑定手机号1
    $("#quick_login1").click(function() {
        var iphone = $("#phone1").val(); //获取输入框里的手机号码
        var code = $("#phone_code1").val(); //获取输入框里的验证码
        var reg = /^1[3578]\d{9}$/;
        if(reg.test(iphone)) {
            if (code == undefined || code == null || code == "") {
                layer.open({
    ...
    // 绑定手机号2
    $("#quick_login2").click(function() {
        var iphone = $("#phone2").val(); //获取输入框里的手机号码
        var code = $("#phone_code2").val(); //获取输入框里的验证码
        var reg = /^1[3578]\d{9}$/;
        if(reg.test(iphone)) {
            if (code == undefined || code == null || code == "") {
                layer.open({
