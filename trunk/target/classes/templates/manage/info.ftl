...
<div class="form-horizontal" role="form" style="margin-top: 20px;">
    <input type="hidden" value="${(info.id)!''}" id="id">
        ...
        <div class="form-group" style="float: left; ">
            ...
            <input type="text" class="form-control" placeholder="标题" maxlength="100" minlength="1" style="width: 400px;float: left;" value="${(info.title)!''}" id="title">
        </div>
        <div class="form-group" style="float: left;margin-left: 20px">
            <div class="col-sm-offset-2 col-sm-10">
             <button class="btn btn-success radius" type="button" onclick="edit()">保存</button>
            </div>
        </div>
    ...
    <br/>
        <div class="form-group" style="float: left;">
            <label style="font-weight:100;font-size:8px;margin-left: 20px">←点击左侧正方形上传图片, 图片大小为1M之内</label>
        </div>
</div>
                ...
                $("#action").ajaxSubmit({
                    ...
                    success: function(data) {
                        if(data.state=='SUCCESS'){
                            picTitle = data.url;
                            $.post('/manage/webSetting/updateInfo',{id:id,title:title,picTitle:picTitle,content:content},function (res) {
                                if(res.count ==1){
                                    layer.msg('保存成功！')
                                    refresh()
                                }else{
