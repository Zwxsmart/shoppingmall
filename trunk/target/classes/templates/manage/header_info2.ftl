...
<div class="form-horizontal" role="form">
    <div >
        <div class="form-group clear my_form-group" style="padding-left: 20px;">
            <label class="control-label">居然之家轮播广告：</label>
...
<script type="text/javascript">
    ...
    function keepAll(size) {
        ...
        for(var j =0; j<size; j++){
            var gotoUrl = $("#carouselGotoUrl"+j).val();
            var id = $("#carouselId"+j).val();
                if($("#carouselChangeId"+j).val()!=''&&$("#carouselChangeId"+j).val()!=undefined&&$("#carouselChangeId"+j).val()!=null){
                    $("#carouselAction"+j).ajaxSubmit({
                        ...
                        success: function(data) {
                            if(data.state=='SUCCESS'){
                                var picUrl = data.url;
                                $.post('/manage/webSetting/updateCarousel',{id:id,gotoUrl:gotoUrl, picUrl:picUrl},function (res) {
                                    if(res.count ==1){
                                    }else{
                    ...
                    $.post('/manage/webSetting/updateCarousel',{id:id,gotoUrl:gotoUrl},function (res) {
                        if(res.count ==1){
                        }else{
                            layer.msg('修改轮播图'+(j+1)+'数据时失败！');
                        }
                        ...
                        $("#carouselAction"+size).ajaxSubmit({
                            ...
                            success: function(data) {
                                if(data.state=='SUCCESS'){
                                    ...
                                    $.post('/manage/webSetting/insertCarousel',{gotoUrl:gotoUrl, picUrl:picUrl1, type:"4", typeName :"居然之家", title:""},function (res) {
                                        if(res.count ==1){
                                        }else{
