...
<script type="text/javascript">
    ...
    function add(){
        ...
        if(title!=undefined&&title!=""&&content!=undefined&&content!=""&&$("#changeId").val()!=''&&$("#changeId").val()!=undefined&&$("#changeId").val()!=null){
                $("#action").ajaxSubmit({
                    type: 'post',
                    dataType: "jsonp",
                    ...
                    success: function(data) {
                        if(data.state=='SUCCESS'){
                            ...
                            picTitle = data.url;
                            $.post('/manage/webSetting/saveOrUpdate2',{id:id,title:title,picTitle:picTitle,content:content,isValid:isValid},function (res) {
                                if(res.count ==1){
                                    layer.msg('添加成功！')
                                    refresh()
                                }else{
